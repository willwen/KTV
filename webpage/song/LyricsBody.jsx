...
export default class LyricsBody extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		nextProps.options.showLineNums
		...
		nextProps.options.showPronounciation
			? (pronounciationStyling = { display: "table-row" })
			: (pronounciationStyling = { display: "none" });
		this.setState({ pronounciationStyling: pronounciationStyling });

		let primaryLangStyling = {};
		nextProps.options.showPrimary
			? (primaryLangStyling = { display: "table-row" })
			: (primaryLangStyling = { display: "none" });
		this.setState({ primaryLangStyling: primaryLangStyling });

		let translatedStyling = {};
		nextProps.options.showTranslated
			? (translatedStyling = { display: "block" })
			: (translatedStyling = { display: "none" });
		this.setState({ translatedStyling: translatedStyling });

		if (nextProps.lyrics) {
			let roundedTimeStamps = nextProps.lyrics.timestamps.map(
				lineTimestamp => {
	...
	render() {
		...
		if (
			this.props.lyrics.pronounciationLanguageLyrics.length > 0 ||
			...
			for (
				...
				var rowDiv = (
					<div
						key={"rowNumber" + lineNumber}
						style={
							this.props.currentLine === lineNumber
								? {
										color:
											Constants.ConstsClass.highlightColor
									}
								: {
										color:
											Constants.ConstsClass
												.foregroundColor
									}
						}
					>
						<div
							className={
								Constants.ConstsClass.lyricLine + "  row"
							}
							id={Constants.ConstsClass.lyricLine + lineNumber}
						>
							<div
								className="col-1 lineIndex vertical-center"
								style={this.state.lineNumberStyling}
							>
								{(lineNumber == 1 || time != "0:00") &&
								time != "NaN:NaN" ? (
									<span>
										<a
											id={"lineNumber" + lineNumber}
											className="lineAnchor"
											onClick={this.props.anchorClickUpdateLine.bind(
												this,
												lineNumber
											)}
										>
							...
							<div
								className="col-11 lyricWords"
								id={
									Constants.ConstsClass.genericLinePrefix +
									lineNumber
								}
							>
								{this.props.primaryLanguage == "Chinese" ? (
												...
												primary[i] ? (
													<tr
														className={
															Constants
																.ConstsClass
																.pronounciationLyricsLineClass
														}
														style={
															this.state
																.pronounciationStyling
														}
													>
														{this.splitPronounciationLang(
															pronounciation[i],
															primary[i],
															lineNumber
														)}
									...
									<div>
										<div
											style={
												this.state.pronounciationStyling
											}
										>
											{" "}
											{pronounciation[i]}
								...
								<div
									className={
										Constants.ConstsClass
											.translatedLyricsLineClass
									}
									style={this.state.translatedStyling}
								>
									{" "}
									{translated[i]}
								</div>
	...
	splitPronounciationLang(pronounciationLine, primaryLine, lineNumber) {
		...
		for (let i = 0; i < pronounciationChars.length; i++) {
			if (primaryLangChars[j] === " ") {
				...
				tdArray.push(
					<td key={lineNumber + "  pronounciationChar " + i}>
						{pronounciationChars[i]}
					</td>
				);
			}
			j++;
	...
	splitPrimaryLang(primaryLine, lineNumber) {
		...
		return primaryLangChars.map((primaryLangChar, index) => {
			return (
				<td key={lineNumber + " primaryLangChar " + index}>
					{primaryLangChar + " "}
				</td>
			);
		});
