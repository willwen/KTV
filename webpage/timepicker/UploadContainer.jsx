export default class UploadContainer extends React.Component {
	...
	audioSourceChanged() {
		...
		this.props.setAudioSource(
			URL.createObjectURL(this.refs.audioSource.files[0])
		);
		window.addEventListener("keydown", e => {
			//this event only fires when file uploaded
		...
		return (
			<div className="dotted-border">
				<div className="row" id="description">
					<p>
						{" "}
						This Time Picker Tool helps record the timestamp for
						each line of a song.{" "}
					</p>
					<p>
						{" "}
						Once you upload the corresponding files, the page will
						be filled with the lyrics and the player with the song.{" "}
					</p>
					<p>
						{" "}
						Press <span className="keys">[Enter]</span> at the start
						of each line; a timestamp is generated to the line's
						right.
					</p>
					<p>
						{" "}
						Use <span className="keys">[ - ]</span> and{" "}
						<span className="keys">[ + ]</span> to control volume.
						Use <span className="keys">[left arrow]</span> and{" "}
						<span className="keys">[right arrow]</span> to seek by 2
						seconds.{" "}
					</p>
					<p>
						{" "}
						Click on the timestamp to edit previously recorded
						timestamps.{" "}
					</p>
					<p>
						{" "}
