...
export default class AudioPlayer extends React.Component {
	...
	componentDidMount() {
		...
		this.refs.audioHTML.volume = 0.5;
		...
		this.setState({
			nextTimestamp: Constants.timestampToSeconds(
				times[this.props.currentLine]
			)
		});
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextTimestamp == 0 && nextProps.currentLine == 0) {
			// fixes a bug if the first time is 0, it isnt highlighted
	...
	checkTimes() {
		if (this.props.timestamps.length != 0) {
			...
			while (nextLineTime == 0) {
				...
				nextLineTime = Constants.timestampToSeconds(
					this.props.timestamps[currentLine + counter]
				);
				counter++;
			}
				...
				if (this.props.allowScrolling) {
					...
					$(window).scrollTo(
						...
						),
						{
							...
							interrupt: true,
							duration: 500,
							offset: { top: this.props.scrollOffset }
						}
					);
				}
			}
	...
	increaseVolume() {
		...
		if (this.refs.audioHTML.volume != 1.0)
			this.refs.audioHTML.volume = this.refs.audioHTML.volume + 0.1;
	}
	...
	decreaseVolume() {
		...
		if (this.refs.audioHTML.volume > 0.1)
			this.refs.audioHTML.volume = this.refs.audioHTML.volume - 0.1;
	}

	setCurrentTime(newLine) {
		let newTime = Constants.timestampToSeconds(
			this.props.timestamps[newLine - 1]
		); //minus one because lyrics start at 0 while currentLine starts at 1
		console.log(newTime);
	...
	calcSeekedLineNumber() {
		...
		for (var i = 0; i < this.props.timestamps.length; i++) {
			if (newSeekedTime < this.props.timestamps[i]) break;
		}
	...
	render() {
		return (
			<div>
				...
				<Visualizer
					audioPlayer={this.refs.audioHTML}
					showVisualizer={this.props.showVisualizer}
				/>
			</div>
		);
	}
